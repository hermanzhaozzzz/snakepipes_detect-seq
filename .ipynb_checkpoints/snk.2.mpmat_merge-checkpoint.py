...
rule all:
    input:
        ...
        expand("../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.filtered.{a}2{b}.mpmat",sample=SAMPLES, rep=REP,a=MPMAT_MERGE[0],b=MPMAT_MERGE[1]),
        expand("../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.filtered.{c}2{d}.mpmat",sample=SAMPLES, rep=REP,c=MPMAT_MERGE[2],d=MPMAT_MERGE[3]),
...
rule mpmat_filter:
    input:
        "../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.select.{ac}2{bd}.mpmat"
    output:
        "../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.filtered.{ac}2{bd}.mpmat"
    params:
        ac = '{ac}',
        bd = '{bd}'
    shell:
...
rule mpmat_merge:
    input:
        "../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.filtered.{a}2{b}.mpmat",
        "../mpmat/293T-bat_{sample}_{rep}_hg38.MAPQ20.filtered.{c}2{d}.mpmat"
    output:
