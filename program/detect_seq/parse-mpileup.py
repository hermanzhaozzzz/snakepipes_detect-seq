...
def split_file_and_make_temp(input_filename, threads_num=1,temp_dir=None):
    ...
    for index in range(threads_num):
        ...
        # open file
        temp_file = open(temp_file_name, "wt")
...
def merge_out_bmat(temp_filename_list, output_filename, remove_temp=True, has_header=False):
    """
    """
    # open output file
    ...
    for temp_filename in temp_filename_list:
        # write file 
        with open(temp_filename + ".bmat","rt") as temp_file:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--HasHeader",
        help="True for Write Header False for no Header",default=False)
    ...
    merge_out_bmat(temp_filename_list=temp_filename_list, output_filename=OUTPUT_FILE_PATH, remove_temp=True, has_header=ARGS.HasHeader)
